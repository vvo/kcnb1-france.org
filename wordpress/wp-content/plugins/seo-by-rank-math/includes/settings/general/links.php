...
$cmb->add_field( array(
	...
	'dep'     => array(
		array( 'attachment_redirect_urls', 'on' ),
	),
) );

$cmb->add_field( array(
	'id'      => 'url_strip_stopwords',
	'type'    => 'switch',
	'name'    => esc_html__( 'Remove Stopwords from Permalinks', 'rank-math' ),
	'desc'    => wp_kses_post( __( 'Remove stopwords such as <strong>a, and, the</strong>, etc. from permalinks. This option will only affect the auto-generated URLs for newly created posts and pages.', 'rank-math' ) ),
	'default' => 'off',
) );
...
$cmb->add_field( array(
	'id'   => 'stopwords',
	'type' => 'textarea_small',
	'name' => esc_html__( 'Stopwords List', 'rank-math' ),
	'desc' => esc_html__( 'List of stopwords to remove. Add one per line.', 'rank-math' ),
	'dep'  => array(
		array( 'url_strip_stopwords', 'on' ),
	),
) );
...
$cmb->add_field( array(
	...
	'dep'  => array(
		array( 'nofollow_external_links', 'on' ),
		array( 'nofollow_image_links', 'on' ),
	),
) );
